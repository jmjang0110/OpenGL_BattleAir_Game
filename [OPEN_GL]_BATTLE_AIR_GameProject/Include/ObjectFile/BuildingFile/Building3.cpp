...
std::vector< glm::vec3 > CBuilding3::m_outvertex;
std::vector< glm::vec3 > CBuilding3::m_outnormal;
std::vector< glm::vec2 > CBuilding3::m_outuv;
GLint* CBuilding3::m_Tri_Num2;
GLint CBuilding3::m_Tri_Num = 1;
...
void CBuilding3::Update_TranslateForm(glm::vec3 translate)
{
...
void CBuilding3::Update_RotateForm(GLfloat Time, GLfloat Axis_x, GLfloat Axis_y, GLfloat Axis_z)
{
...
void CBuilding3::Update_ScaleForm(GLfloat sx, GLfloat sy, GLfloat sz)
{
...
void CBuilding3::Update_ModelTransform(float fDeltaTime)
{
...
glm::vec3 CBuilding3::GetCollide_Position(int idx)
{
...
void CBuilding3::Init(glm::vec3 scaleInfo, glm::vec3 color, glm::vec3 pivot, const char* filename, stbi_uc* textData, stbi_uc* textData2,
	int text_building_width, int text_building_height, int textRed_height_width, int textRed_height_height)
{
	m_Building3_Text_data = textData;

	m_Pivot = pivot;
	m_Color = color;
	m_Pivot.y += 12.0f;
	m_Pivot.y = rand() % 200 + 20.0f;

	m_CollideBox = new Chexahedron;
	m_CollideBox->Init(scaleInfo.x * 1.0f, scaleInfo.y * 1.0f, scaleInfo.z * 2.0f, m_Pivot, textData2, textRed_height_width, textRed_height_height);
	...
	InitTexture_1(m_Building3_Text_data, text_building_width, text_building_height);
	InitBuffer();

}

void CBuilding3::Input(float fDeltaTime)
{
}

int CBuilding3::Update(float fDeltaTime)
{
...
int CBuilding3::LateUpdate(float fDeltaTime)
{
	return 0;
}

void CBuilding3::Collision(float fDeltaTime)
{
}

void CBuilding3::Render(float fDeltaTime)
{
...
void CBuilding3::InitTexture_1(stbi_uc* textData, int text_building_width, int text_building_height)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, text_building_width, text_building_height, 0, GL_RGB, GL_UNSIGNED_BYTE, m_Building3_Text_data);
...
int CBuilding3::loadObj_normalize_center(const char* filename)
{
	FILE* objFile = NULL;
	static int i = 0;
	cout << i++ << endl;
