...
std::vector< glm::vec3 > CBackground::m_outvertex;
std::vector< glm::vec3 > CBackground::m_outnormal;
std::vector< glm::vec2 > CBackground::m_outuv;
GLint* CBackground::m_Tri_Num2;
GLint CBackground::m_Tri_Num = 1;
...
void CBackground::Update_TranslateForm(glm::vec3 translate)
{
...
void CBackground::Update_RotateForm(GLfloat Time, GLfloat Axis_x, GLfloat Axis_y, GLfloat Axis_z)
{
...
void CBackground::Update_ScaleForm(GLfloat sx, GLfloat sy, GLfloat sz)
{
...
void CBackground::Update_ModelTransform(float fDeltaTime)
{
...
void CBackground::Init(glm::vec3 scaleInfo, glm::vec3 color, glm::vec3 pivot, const char* filename)
{
...
void CBackground::Render(float fDeltaTime)
{
	glBindVertexArray(m_VAO);
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

	// 안쪽 면 보려고 CULL FACE 해제 했음
	// -> 다른 오브젝트 그릴때 CULL FACE 사용할거면 glenable 처리 해야 함
...
int CBackground::loadObj_normalize_center(const char* filename)
{
	FILE* objFile = NULL;
	static int i = 0;
	cout << i++ << endl;
